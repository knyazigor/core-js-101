...
function getRectangleString(width, height) {
  ...
  for (let y = 0; y < height; y += 1) {
    let line = '';
    if (y === 0) line = `┌${'─'.repeat(width - 2)}┐\n`;
    if (y > 0 && y < height - 1) line = `│${' '.repeat(width - 2)}│\n`;
    if (y === height - 1) line = `└${'─'.repeat(width - 2)}┘\n`;
    result += line;
  }
  return result;
}
...
function encodeToRot13(str) {
  ...
  return str.split('').map((el) => {
    ...
    if (alpha.includes(el.toLowerCase())) {
      result = alpha.charAt((alpha.indexOf(el.toLowerCase()) + 13) % 26);
    }
...
function getCardId(value) {
  ...
  return ranks.indexOf(rank) + (13 * suits.indexOf(suite));
}
